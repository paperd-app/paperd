...
| MLX移行 | embedding（および将来的にDocling代替）をMLXベースへ移行し、Python環境を軽量化 | [06](06-search-rag.md) |
...
| # | リスク | 影響 | 対策 |
|---|---|---|---|
| R1 | Python環境のサイズ（Docling + PyTorchで2〜3GB）と初回セットアップの失敗 | 初回体験の悪化、サポート負荷 | セットアップウィザードで進捗・中断再開・オフライン明示（[09](09-ui.md)）。将来のMLX移行で根本改善（v2） |
...
| U3 | bge-m3のONNX / MLX変換版採用によるPyTorch依存削減の可否 | M3で性能計測後 |
