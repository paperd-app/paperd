...
graph TB
  ...
  subgraph Worker["paperd-worker (Python)"]
    ...
    EMB[Qwen3-Embedding MLX<br/>embedding生成]
  end
...
DoclingとMLX embedding runtimeをユーザ環境に構築するため、アプリバンドルへの`.venv`同梱はしない。

1. 設定画面「環境構築（venv + pip install）」ボタンで、`<workerDir>/.venv` を `python -m venv` で作成し、`.venv/bin/pip install --upgrade -e .[ml]` で Docling / MLX / Qwen3 embedding runtime を同期する
2. ワーカーは `<workerDir>/.venv/bin/python -m paperd_worker --token ... --port 0 ...` で起動する（`paperd_worker/__main__.py` がエントリ）
3. embedding モデル（Qwen3-Embedding-0.6B 4bit）は初回利用時にHugging Face cacheへダウンロード
...
依存ライブラリ（GRDB / Docling / Qwen3-Embedding / MLX / ocrmac等）はすべて商用利用可能なライセンスのものを採用する。
...
| MLX + Qwen3-Embedding-0.6B 4bit | embedding生成 | MIT / Apache-2.0 |
